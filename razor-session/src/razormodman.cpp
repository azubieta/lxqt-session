      ...
      mThemeWatcher(new QFileSystemWatcher(this))
{
    ...
    connect(LxQt::Settings::globalSettings(), SIGNAL(razorThemeChanged()), SLOT(themeChanged()));
...
void RazorModuleManager::themeFolderChanged(const QString& /*path*/)
{
    ...
    {
        const QList<LxQt::RazorTheme> &allThemes = LxQt::razorTheme.allThemes();
    ...
    {
        newTheme = LxQt::razorTheme.currentTheme().name();
    }

    LxQt::Settings settings("razor");
...
void RazorModuleManager::themeChanged()
{
    ...
    if (LxQt::razorTheme.currentTheme().isValid())
    {
        mCurrentThemePath = LxQt::razorTheme.currentTheme().path();
        mThemeWatcher->addPath(mCurrentThemePath);
    }
}

void RazorModuleManager::startWm(LxQt::Settings *settings)
{
    ...
    int waitCnt = 300;
    while (!LxQt::xfitMan().isWindowManagerActive() && waitCnt)
    {
        waitCnt--;
        QCoreApplication::processEvents();
        usleep(100000);
